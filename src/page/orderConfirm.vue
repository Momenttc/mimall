        ...
        <div class="order-box">
          <div class="item-address">
            ...
            <div class="addr-list clearfix">
              <div class="addr-info" v-for="(item,index) in List" :key="index" :class="{'checked':index==checkedIndex}" @click="checkedIndex=index">
              ...
              <div class="addr-add">
                <div class="icon-add" @click="addAddress"></div>
                <div>添加新地址</div>
              </div>
            </div>
          </div>
          ...
          <div class="btn-group">
            ...
            <a href="javascript:;" class="btn btn-large" @click=orderSettlement>去结算</a>
          </div>
          <modal
            title="新增地址"
            btnType="1"
...
export default {
  ...
  data() {
    return {
      List: [], //收获地址列表
      CartList: [], //获取已经选中的商品列表
      cartTotalPrice: 0,
      count: 0, //已添加的商品总件数
      operationType: "", //地址操作类型，0：添加，1：编辑，3：删除
      checkedItem: {}, //选中的商品对象
      showDelModal: false, //是否显示确认删除弹框
      showAddModal: false, //是否显示新增地址弹框
      checkedIndex: 0
    };
  },
  components: {
  ...
  methods: {
    ...
    getCartList() {
      this.axios.get("/carts").then(res => {
        let allCartList = res.cartProductVoList;
        this.CartList = allCartList.filter(item => item.productSelected);
